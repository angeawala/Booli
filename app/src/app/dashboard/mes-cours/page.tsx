...
import '@/styles/dashboard/mes-cours.css';

// Simuler l'utilisateur connecté (admin ou staff)
...
const MyCoursesPage = () => {
  ...
  const handleApproveCourse = (courseId: string, approve: boolean) => {
    setLoading(true);
    setTimeout(() => {
      const updatedCourses = courses.map((course) =>
        course.id === courseId ? { ...course, isApproved: approve, lastUpdated: new Date().toISOString() } : course
      );
      setCourses(updatedCourses);
      saveCourses(updatedCourses);
      toast.success(approve ? 'Cours approuvé avec succès' : 'Cours désapprouvé avec succès');
      setLoading(false);
    }, 1000);
  };
  ...
  return (
    <div className="x-my-courses-page">
      <ToastContainer position="top-right" autoClose={3000} />
      <Loader loading={loading} />
      <h2 className="x-my-courses-title">Gestion des Formations</h2>
        ...
        <table className="table table-striped">
          ...
          <tbody>
            {courses.map((course) => (
              <tr key={course.id}>
                <td>{course.title}</td>
                <td>{course.creatorEmail} {course.specifiedAuthor ? `(${course.specifiedAuthor})` : ''}</td>
                ...
                <td>
                  ...
                  <span className="x-action-icon" onClick={() => handleApproveCourse(course.id, !course.isApproved)}>
                    <i className={course.isApproved ? 'fas fa-times' : 'fas fa-check'}></i>
                    <span className="x-action-tooltip">{course.isApproved ? 'Désapprouver' : 'Approuver'}</span>
                  </span>
      ...
      {selectedCourse && !selectedModule && (
        <div className="x-manage-modules">
          <h3 className="x-section-title">Gérer les Modules de "{selectedCourse.title}"</h3>
          <div className="mb-4">
      ...
      {/* Gérer les Chapitres et le Quiz d’un Module */}
      {selectedCourse && selectedModule && (
        <div className="x-manage-content">
          <h3 className="x-section-title">Gérer le Contenu de "{selectedModule.title}"</h3>
