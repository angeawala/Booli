  ...
  const handleCreateProduct = (formData: Product) => {
    ...
    setTimeout(() => {
      const newProduct: Product = {
        ...
        variants: formData.variants || [], // Ensure variants is always an array
      };
  ...
  const handleAddBulkPricing = () => {
    ...
    setTimeout(() => {
      const updatedProducts = products.map((product) => {
        ...
        const updatedVariants = product.variants?.map((variant, index: number) => {
      ...
      saveProducts(updatedProducts as Product[]);
        ...
        <>
          <div className="card mb-4">
            <div className="card-body">
              <h3>{userShop.name}</h3>
              <p>
                <strong>Email :</strong> {userShop.email}
              </p>
              <p>
                <strong>Contact :</strong> {userShop.contact}
              </p>
              <p>
                <strong>Description :</strong> {userShop.description}
              </p>
              <p>
                        ...
                        product.variants && product.variants.length > 0 ? (
                          product.variants.map((variant, index: number) => (
                            <tr key={`${product.id}-${index}`}>
                              <td>{product.name}</td>
                              <td>
                                {`${variant.size} (${variant.colors
                                  .map((c) => c.color)
                                  .join(', ')})`}
                              </td>
                              <td>
                                {variant.promotion ? (
                                  `${variant.promotion.discountPercentage}% (du ${
                                    variant.promotion.startDate
                                  } au ${variant.promotion.endDate})`
                                ) : (
                                  'Aucune'
                                )}
                              </td>
                              <td>
                                {variant.bulkPricing && variant.bulkPricing.length > 0 ? (
                                  variant.bulkPricing
                                    .map(
                                      (bp) =>
                  ...
                  {shopProducts
                    ...
                    ?.variants?.map((variant, index: number) => (
                      <option key={index} value={index}>
                        {`${variant.size} (${variant.colors.map((c) => c.color).join(', ')})`}
                      </option>
                    ))}
                </select>
              </div>
