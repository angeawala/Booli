...
const VendorOrdersPage = () => {
  ...
  const userEmail = 'vendor@example.com'; // Simulated logged-in vendor
  ...
  useEffect(() => {
    ...
    const vendorShop = allShops.find((shop) => shop.ownerEmail === userEmail);
    if (vendorShop) {
      const vendorOrders = getOrders().filter(
        (order) => order.shopId === vendorShop.id
      );
  ...
  const handleStatusChange = (orderId: string, newStatus: string) => {
    if (newStatus === 'canceled' || newStatus === 'disputed') {
      toast.error('Seul le staff peut annuler ou marquer une commande en litige.');
      return;
    }
    setLoading(true);
    setTimeout(() => {
  ...
  return (
    <div className="x-orders-page">
      ...
      <h2 className="x-orders-title">Gestion des Commandes (Vendeur)</h2>
